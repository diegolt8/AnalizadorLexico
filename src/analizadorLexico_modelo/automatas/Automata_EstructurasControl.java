...
public class Automata_EstructurasControl implements Automata {
    ...
    public Lexema ejecutarAutomata(FlujoCaracteres flujo) {
        ...
        while (flujo.getPosicionActual() < flujo.getCantidadCaracteres()) {
            ...
            if ("qf".equals(estado)) {
                if("Si".equals(lexema)){
                    if('n' == flujo.getCaracter()){
                        estado = "q16";
                        continue;
                    }
                }
                if("Para".equals(lexema)){
                    if('C' == flujo.getCaracter()){
                        estado = "q18";
                        continue;
                    }
                }
    ...
    public Lexema estado_qf(String lexema, int fila, int columna) {
        ...
        switch (lexema){
            case "Si":
                tipoLexema = TipoLexemaEnum.EST_SI;
                break;
            case "Para":
                tipoLexema = TipoLexemaEnum.EST_PARA;
                break;            
            case "Hacer":
                tipoLexema = TipoLexemaEnum.EST_HACER;
                break;
            case "Mientras":
                tipoLexema = TipoLexemaEnum.EST_MIENTRAS;
                break;
            case "Sino":
                tipoLexema = TipoLexemaEnum.EST_SINO;
                break;
            case "ParaCada":
                tipoLexema = TipoLexemaEnum.EST_PARA_CADA;
                break;
        }
